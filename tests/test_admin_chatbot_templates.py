...
def test_admin_chatbot_templates_list_and_apply_publish(client, db_session):
    ...
    r1 = client.get("/admin/re/chatbot-templates", headers=headers)
    assert r1.status_code == 200, r1.text
    assert any(x.get("domain") == "real_estate" and x.get("template") == "default" for x in r1.json())
    assert any(x.get("domain") == "car_dealer" and x.get("template") == "default" for x in r1.json())

    r2 = client.post(
        "/admin/re/chatbot-templates/apply",
        json={"domain": "real_estate", "template": "default", "name": "default", "overwrite": True, "publish": True},
        headers=headers,
    )
    ...
    flow2_id = int(r3.json().get("flow_id") or 0)
    assert flow2_id > 0
    assert int(r3.json().get("published_version") or 0) >= 1
    ...
    assert int(published2.id) == flow2_id
